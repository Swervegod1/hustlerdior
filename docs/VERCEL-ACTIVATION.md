...
| `PRINTFUL_API_TOKEN` | Private Printful token for the source store |
| `PRINTFUL_STORE_ID` | Store actually authorized by the token; retain the verified production value |
| `DATABASE_URL` | Pooled Neon URL with provider-required TLS, supplied through secure environment settings |
| `APP_SESSION_SECRET` | Cryptographically random secret, at least 32 characters |
...
| `STRIPE_TAX_MODE` | `automatic` in live mode; settings/default classification and applicable registrations must be reviewed |
| `CRON_SECRET` | Separate cryptographically random secret, at least 32 characters |
| `CHECKOUT_ENABLED` | `false` until configuration, quote/payment tests and worker verification pass; then `true` |

This application uses neither NextAuth nor client-side Stripe Elements. `NEXTAUTH_SECRET`, `NEXTAUTH_URL`, `NEXT_PUBLIC_SITE_URL`, and a publishable Stripe key do not unlock its existing checkout. The Printful variable is `PRINTFUL_API_TOKEN`, not `PRINTFUL_API_KEY`.
