...
import { orderSnapshotSchema } from "@/lib/commerce-schema";
...
export async function POST(request: Request) {
  ...
  try {
    await transaction(async (client) => {
      ...
      if (!order) return;
      const snapshot = orderSnapshotSchema.parse(order.snapshot);
      if (
        event.livemode !== snapshot.livemode ||
        session.livemode !== snapshot.livemode ||
    ...
    // Durable commit happens before acknowledgement; fulfillment belongs to the worker.
    return Response.json({ received: true }, { headers: privateHeaders });
  } catch {
    return Response.json(
