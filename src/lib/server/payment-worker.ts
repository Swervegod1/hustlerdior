...
import { loadOrder, recoverOrder, saveOrder, type OrderRecord } from "./orders";
...
export async function processPaidOrder(
  id: string,
  sessionHint?: Stripe.Checkout.Session,
) {
  const order: OrderRecord = sessionHint?.metadata
    ? await recoverOrder(id, sessionHint.metadata)
    : await loadOrder(id);
  if (!order.stripe_session_id && !sessionHint?.id)
  ...
  const session = await stripe.checkout.sessions.retrieve(
    order.stripe_session_id || sessionHint!.id,
  );
  ...
  order.stripe_payment_id = paymentId;
  order.status = "paid";
  await saveOrder(order);
  ...
  order.printful_order_id = String(draft.id);
