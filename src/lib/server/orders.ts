...
export async function loadOrder(
  ...
  const result = await database().query(
    "SELECT * FROM hd_orders WHERE id=$1" + (owner ? " AND owner_hash=$2" : ""),
    owner ? [id, owner] : [id],
  );
  const row = result.rows[0];
  if (!row) throw new HttpError(404, "Order not found in this browser.");
  return { ...row, snapshot: orderSnapshotSchema.parse(row.snapshot) };
}
const cost = z
  .string()
  .regex(/^\d+(\.\d{1,2})?$/)
...
export async function quoteOrder(input: unknown, owner: string) {
  if (!checkoutConfigured())
    throw new HttpError(
      503,
      "Online checkout is being prepared. Your bag is saved.",
    );
  ...
  if (!Number.isFinite(floor) || floor < 0 || floor > 0.8)
    throw new HttpError(503, "Checkout is being prepared.");
  ...
  await database().query(
    "INSERT INTO hd_orders(id,owner_hash,snapshot) VALUES ($1,$2,$3::jsonb)",
    [id, owner, JSON.stringify(snapshot)],
  );
  return {
    id,
...
export async function createCheckout(id: string, owner: string) {
  ...
  await database().query(
    "UPDATE hd_orders SET stripe_session_id=$2,status=CASE WHEN status='quoted' THEN 'checkout' ELSE status END,updated_at=now() WHERE id=$1 AND (stripe_session_id IS NULL OR stripe_session_id=$2)",
    [id, session.id],
  );
