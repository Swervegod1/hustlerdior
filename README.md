...
**Activation status (September 18, 2026):** this branch restores the complete saved storefront over the older catalog-only GitHub scaffold. It includes the checkout backend, SQL migrations, and an authenticated Vercel payment-worker route. Build, type checks, lint and commerce tests pass locally. Production checkout is not yet enabled; deployment, Stripe runtime credentials, tax configuration and domain routing remain to be completed. See [the activation runbook](docs/VERCEL-ACTIVATION.md).
...
- Live delivery quote flow, margin guard, Stripe hosted Checkout, signed payment webhooks and a durable paid-order-to-Printful-draft worker.
...
- Real sanitized snapshot of **100 products / 2,444 variants** from store **18747907**.
...
Set `PRINTFUL_API_TOKEN` in `.env.local`. Provision the private token securely; it is intentionally not included in source control. Set `PRINTFUL_STORE_ID=18747907` and keep `SITE_URL=http://localhost:3000` locally.
...
| Route                                 | Behavior                                                                             |
| ------------------------------------- | ------------------------------------------------------------------------------------ |
| `GET /api/products?limit=12&offset=0` | Normalized live catalog and a pagination cursor; limit 1–24                          |
...
| `POST /api/webhooks/stripe` | Verifies signatures and queues durable payment work |
...
| `GET /api/health`                     | Runtime health response                                                              |
